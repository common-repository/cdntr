...
final class CDNTR_Engine {
    ...
    public function __construct() {
        ...
        self::$settings = CDNTR::get_settings();
    ...
    private static function is_admin() {
        ...
        if ( apply_filters( 'cdntr_exclude_admin', is_admin() ) ) {
            return true;
        }
    ...
    private static function bypass_rewrite() {
        ...
        // bypass rewrite hook
        if ( apply_filters( 'cdntr_bypass_rewrite', false ) ) {
            return true;
        }
    ...
    private static function rewrite_url( $matches ) {
        ...
        $file_url       = $matches[0];
        $site_hostname  = ( ! empty( $_SERVER['HTTP_HOST'] ) ) ? self::sanitize_server_input( $_SERVER['HTTP_HOST'] ) : wp_parse_url( home_url(), PHP_URL_HOST );
        $site_hostnames = (array) apply_filters( 'cdntr_site_hostnames', array( $site_hostname ) );
        ...
        if ( apply_filters( 'cdntr_rewrite_relative_urls', true ) ) {
    ...
    public static function rewriter( $contents ) {
        ...
        $contents = apply_filters( 'cdntr_contents_before_rewrite', $contents );
        ...
        $rewritten_contents = apply_filters( 'cdntr_contents_after_rewrite', preg_replace_callback( $urls_regex, self::class . '::rewrite_url', $contents ) );
